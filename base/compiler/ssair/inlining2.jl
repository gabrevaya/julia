...
function ir_inline_unionsplit!(compact::IncrementalCompact, idx::Int,
    ...
    for ((metharg, case), next_cond_bb) in zip(item.cases, item.bbs)
        ...
        elseif isa(case, MethodInstance)
            val = insert_node_here!(compact, Expr(:invoke, case, argexprs...), Union{}, line)
        else
...
function analyze_method!(ir, idx, f, ft, metharg, methsp, method, stmt, atypes, sv, atype_unlimited, isinvoke, isapply, invoke_data)
    methsig = method.sig
    ...
    if !isa(linfo, MethodInstance)
        return maybe_make_invoke!(ir, idx, stmt.typ, atypes, sv, atype_unlimited, isinvoke, isapply, invoke_data)
    end
...
function assemble_inline_todo!(ir::IRCode, linetable::Vector{LineInfoNode}, sv::OptimizationState)
    ...
    for idx in 1:length(ir.stmts)
        ...
        if invoke_data !== nothing
            ...
            result = analyze_method!(ir, idx, f, ft, metharg, methsp, method, stmt, atypes, sv, atype_unlimited, isinvoke, isapply, invoke_data)
        ...
        for (i, match) in enumerate(meth)
            ...
            case = analyze_method!(ir, idx, f, ft, metharg, methsp, method, stmt, atypes, sv, metharg, isinvoke, isapply, invoke_data)
            if case === nothing
                fully_covered = false
                continue
            end
