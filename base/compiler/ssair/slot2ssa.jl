...
function construct_ssa!(ci::CodeInfo, code::Vector{Any}, ir::IRCode, domtree::DomTree, defuse, nargs::Int)
    ...
    @timeit "idf" for (idx, slot) in Iterators.enumerate(defuse)
        ...
        for block in phiblocks
            ...
            node = PhiNode()
            ssa = insert_node!(ir, first_insert_for_bb(code, cfg, block), Union{}, node)
    ...
    @timeit "SSA Rename" while !isempty(worklist)
        ...
        for (idx, slot) in Iterators.enumerate(phi_slots[item])
            ...
            ir.new_nodes[new_node_id] = NewNode(old_entry, typ=new_typ, node=node)
    ...
    for (_, nodes) in phicnodes
        for (_, ssa, node) in nodes
            ...
            ir.new_nodes[new_idx] = NewNode(node, typ=new_typ)
        end
    end
    ...
    new_nodes = NewNode[let node = new_nodes[i]
            NewNode(node, node=new_to_regular(renumber_ssa!(node.node, ssavalmap)))
